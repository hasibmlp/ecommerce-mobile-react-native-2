...
export function CollectionCard({ product, onPress }) {
  ...
  return (
    <View key={product.id} className=" w-full justify-center mr-[10px] ">
      ...
      >
        <Text className="text-[12px] text-black font-medium uppercase mb-2">
          {product.vendor}
        </Text>
        {product.title && (
          <Text
        ...
        {product.options[0].values[0] !== "Default Title" &&
          ...
          product.options.find((op) => op.name === "Color")?.values.length >
            1 && <ColorSwatchesContainer product={product} />}
      </Pressable>
    </View>
  );
}
            ...
            colorOption.map((item, index) => (
              ...
              >
                ...
                <View className="py-1 items-center">
                  <Text className="text-[14px] text-balck font-normal pb-2">
                    {item.value}
                  </Text>
