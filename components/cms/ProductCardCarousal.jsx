    ...
    return (
      <View className={`bg-white py-[15px] pb-[40px] flex-col mb-3`}>
        <View className="flex-row justify-between py-[10px] px-[15px]">
          ...
          {data && (<Text className="text-[18px] font-light text-black capitalize">
            {data.collection.title}
          </Text>)}
          <Pressable
            onPress={() =>
              navigation.navigate("Collection", { collectionId: content.data.collectionId })
            }
          >
        ...
        >
            <View className="flex-row px-2">
                {data && data.collection.products.edges?.map((product, index) => (
                    <TouchableOpacity
                    key={index.toString()}
                    onPress={() => {
                ...
                {!data && loading && (
                    ...
                    <View className="items-center mr-2">
                        ...
                        <Skeleton width={130} height={11} rounded style={{marginTop: 8}} />
                        <Skeleton width={80} height={11} rounded style={{marginTop: 8}} />
                    </View>
                    <View className="items-center mr-2">
                        <Skeleton width={150} height={230} />
                        <Skeleton width={130} height={11} rounded style={{marginTop: 8}} />
