...
const ImageSelection = ({style, title, onChange, defaultValue, handleChange, images}) => {
    ...
    return (
      <View style={style} className="w-full">
        ...
        <View className="flex-row justify-center">
          <ImageSelectionCard imageUrl={images[0]?.url} label={images[0]?.title}  onPress={() => handlePress('left')} active={active === 'left'}/>
          <ImageSelectionCard imageUrl={images[1]?.url} label={images[1]?.title} onPress={() => handlePress('right')} active={active === 'right'} style={{marginLeft: 12}} />
        </View>
      </View>
    )
}
  ...
  return (
    <Pressable style={style} className="items-center" onPress={onPress}>
        ...
        <View className="flex-row mt-2 items-center ">
          {label && (<Text className="text-[14px] text-black font-normal uppercase mr-2">{label}</Text>)}
