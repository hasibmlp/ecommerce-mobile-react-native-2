  ...
  if (images && selectedOption !== null) {
    selectedVariantImages = images.filter((image) => {
      if (image.alt) {
        ...
        const colorSubstring = substrings.find((substring) =>
          substring.startsWith("color_")
        );
        const colorValue = colorSubstring
          ? colorSubstring.substring("color_".length)
          : null;
        if (colorValue === selectedOption?.toLowerCase().replace(/\s+/g, "_"))
          return image;
      }
    });
  }
...
export {
  filterActiveInputValues,
  deepEqual,
  isObject,
  isActiveFilterInputMatchesWithValue,
  checkTabActive,
  isFilterValueActive,
  getVariantImages,
