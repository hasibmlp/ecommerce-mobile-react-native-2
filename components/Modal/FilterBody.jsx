...
export default function FilterBody({onClose}){
    const {loading} = useContext(FilterSelectionContext)
    return (
...
function Content ({openSideBar}) {
    const {filters, activeFilterInput, setLoading} =  useContext(FilterSelectionContext)
...
function Footer ({onClose}) {
    ...
    const {activeFilterInput, loading} = useContext(FilterSelectionContext)
    ...
    return (
        <View style={{backgroundColor:'#fff', shadowColor:'#000', shadowOffset:{width:0, height:-4}, shadowOpacity:0.05, elevation:3}} className="w-full items-center py-3 mb-3">
