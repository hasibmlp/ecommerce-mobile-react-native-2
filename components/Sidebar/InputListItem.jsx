...
export default function InputListItem({style, priceType, values, handleBlur, handleChange}) {
    ...
    return (
        <View style={[style]} className="">
            ...
            <PriceInput label={`${priceType === 'min' ? 'Min' : 'Max'} AED 2000`} currencyCode="AED" error={error} values={priceType === 'min' ? values.min : values.max} handleBlur={handleBlur(priceType === 'min' ? 'min' : 'max')} handleChange={handleChange(priceType === 'min' ? 'min' : 'max')}/>
        </View>
    )
}
