...
} from "../components/utils/UtilsFunctions";
...
function PreVariantSelectionProvider({ children, productId, handleClose }) {
  ...
  const firstSelectedColor = options?.find((option) => option.name === "Color")
    ?.values[0];
  let initialColorOption;
  if (firstSelectedColor) {
    const variant = getVariantForSingleOption(
      variants,
      "Color",
      firstSelectedColor
    );
  ...
  const isCurrentlyInStock = () => {
    ...
    const option = [
      { name: "Color", value: activeColor?.value },
      { name: "Size", value: activeSize },
